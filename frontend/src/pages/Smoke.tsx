...
function initialState(spec: EndpointSpec): EndpointState {
  ...
  return { pathParams, body: spec.defaultBody ?? '', result: null, loading: false };
}
...
export function Smoke() {
  ...
  async function ping(spec: EndpointSpec) {
    ...
    try {
      ...
      update(spec.id, { loading: false, result: { status: res.status, ok: res.ok, body: pretty, ms } });
    } catch (err) {
      const ms = Math.round(performance.now() - t0);
      update(spec.id, { loading: false, result: { error: (err as Error).message || String(err), ms } });
    }
  }
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <h1>Endpoint smoke test</h1>
        <p>
          Pings every <code>/api/*</code> route on the NestJS backend. A 4xx response still counts
          as "reachable" — the goal is to confirm Vite + CORS + the backend are all talking.
        </p>
      </div>
              ...
              return (
                <article className="card" key={spec.id}>
                  ...
                  <div className="actions">
                    <button className="btn" onClick={() => ping(spec)} disabled={s.loading}>
